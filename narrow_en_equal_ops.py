...
for i in inds: 
	...
	psi0 = psi0 + 1.0/np.sqrt(num)*eigvecs[:, i]
...
ops_initial = [np.dot(np.conjugate(psi0), np.dot(ops[i], psi0)) for i in range(len(ops))] 
...
with open('all_ops_diag_approx_'+str(L)+'_E_'+str(E)+'_delta_'+str(delta)+'_equal_sym'+'_cx_'+str(cx)+'_cy_'+str(cy)+'_cz_'+str(cz)+'.npy', 'wb') as g:
	np.save(g, ops_diag)
...
def half_line_time_ev(times, label): 
	...
	with open('allopstimes_'+str(L)+'_E_'+str(E)+'_delta_'+str(delta)+'_equal_sym_maxtime_'+label+'_cx_'+str(cx)+'_cy_'+str(cy)+'_cz_'+str(cz)+'.npy', 'wb') as f:
